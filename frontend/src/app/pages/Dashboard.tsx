...
async function generarReporteGeneralPDF(params: {
  ...
  let y = 42;
      subtitle:
        totalPrestado > 0
          ? `${((totalRecuperado / totalPrestado) * 100).toFixed(1)}% del capital`
          : "0.0% del capital",
  // Summary section
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        ...
        "Deudas Finalizadas",
        String(params.deudas.filter((d) => d.estado === "pagada").length),
      ],
    ],
    theme: "grid",
    headStyles: {
  ...
  // Active debts table
  const activeDebts = params.deudas.filter((d) => d.estado === "activa");
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(15, 23, 42);
  ...
  autoTable(doc, {
    startY: y,
    head: [
      ["Cliente", "Descripción", "Monto", "Saldo", "Tasa", "Abonos", "Estado"],
    ],
    body: activeDebts.map((d) => {
      const cliente = params.clientes.find((c) => c.id === d.clienteId);
      ...
      return [
        cliente?.nombre ?? "—",
        d.descripcion.length > 28
          ? d.descripcion.substring(0, 28) + "…"
          : d.descripcion,
  ...
  for (let i = 1; i <= totalPagesN; i++) {
    ...
    doc.text(
      `CreditLine · ClustLayer · Página ${i} de ${totalPagesN}`,
      pageWidth / 2,
  ...
  doc.save(
    `reporte_general_creditline_${new Date().toISOString().slice(0, 10)}.pdf`,
  );
...
export function Dashboard() {
  ...
  const totalPrestado = deudas.reduce((s, d) => s + d.monto, 0);
  const totalRecuperado = deudas.reduce(
    (s, d) => s + d.abonos.reduce((a, b) => a + b.monto, 0),
    0,
  );
  const totalIntereses = deudas
    .filter((d) => d.estado === "activa")
    .reduce((s, d) => s + calcularInteresesGenerados(d), 0);
  const deudasActivas = deudas.filter((d) => d.estado === "activa").length;
  const deudasAtrasadas = deudas.filter(
  ...
  const last6MonthsData = useMemo(() => {
    ...
    return Array.from({ length: 6 }, (_, i) => {
      ...
      const abonado = deudas
        .flatMap((debt) => debt.abonos)
        .filter((abono) => {
      ...
      const mesLabel =
        year < 2026 ? `${MESES_LABELS[month]}'25` : MESES_LABELS[month];
  ...
  const metrics = [
    {
      ...
      subtitle: `${deudas.length} préstamos registrados`,
      icon: DollarSign,
      color: "#2563EB",
    },
    {
      ...
      subtitle: `${((totalRecuperado / totalPrestado) * 100).toFixed(1)}% del capital`,
      icon: TrendingUp,
      color: "#16A34A",
    },
    {
      title: "Intereses Activos",
      value: formatCOP(totalIntereses),
      subtitle: "Intereses acumulados",
      icon: Banknote,
      color: "#F59E0B",
    },
    {
      title: "Saldo por Cobrar",
      value: formatCOP(saldoTotal),
      subtitle: "Capital + intereses pendientes",
      icon: CreditIcon,
      color: "#1E3A8A",
    },
    {
  ...
  // Recent movements
  const recentAbonos = deudas
    .flatMap((d) =>
      d.abonos.map((a) => ({
        ...a,
        clienteId: d.clienteId,
        deudaId: d.id,
        deudaDesc: d.descripcion,
      })),
    )
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 lg:px-6 py-8">
      {loading && (
        <div className="mb-4 text-[#64748B] text-sm">
      ...
      <motion.div
        ...
        className="bg-white rounded-2xl shadow-sm border border-[#E2E8F0] p-5 lg:p-6 mb-8"
      >
        <div className="mb-4">
          <h3 className="text-[#0F172A]">Flujo Mensual — Últimos 6 Meses</h3>
          <p className="text-[#64748B] text-sm">
            Dinero prestado vs. dinero abonado por mes
          </p>
        </div>
          ...
          <BarChart data={last6MonthsData} barGap={4} barSize={18}>
            ...
            <RechartTooltip
              formatter={(value: number, name: string) => [
                ...
                name === "prestado" ? "Prestado" : "Abonado",
              ]}
              contentStyle={{
                borderRadius: 12,
            ...
            <Legend
              ...
              formatter={(value) =>
                value === "prestado" ? "Prestado" : "Abonado"
              }
            />
            <Bar
              dataKey="prestado"
      ...
      >
        <div className="flex items-center justify-between mb-5">
          ...
          <Link
            to="/deudas"
            className="text-[#2563EB] text-sm hover:text-[#1E3A8A] transition-colors flex items-center gap-1"
          >
              ...
              >
                <thead>
                  <tr className="border-b border-[#E2E8F0]">
                    <th className="text-left py-3 px-3 text-[#64748B] text-xs font-medium">
                      Cliente
                    </th>
                    <th className="text-left py-3 px-3 text-[#64748B] text-xs font-medium hidden sm:table-cell">
                      Concepto
                    </th>
                    <th className="text-left py-3 px-3 text-[#64748B] text-xs font-medium">
                      Tipo
                    </th>
                    <th className="text-right py-3 px-3 text-[#64748B] text-xs font-medium">
                      Monto
                    </th>
                    <th className="text-right py-3 px-3 text-[#64748B] text-xs font-medium hidden md:table-cell">
                      Fecha
                    </th>
                  </tr>
                </thead>
                  ...
                  {recentAbonos.map((mov) => (
                    ...
                    >
                      <td className="py-3 px-3">
                        <Link
                          to={`/cartera/${mov.clienteId}`}
                          className="text-[#0F172A] text-sm hover:text-[#2563EB] transition-colors"
                        >
                          {getClienteName(mov.clienteId)}
                        </Link>
                      </td>
                      <td className="py-3 px-3 text-[#64748B] text-sm truncate max-w-[140px] hidden sm:table-cell">
                        {mov.deudaDesc}
                      </td>
                      <td className="py-3 px-3">
                        <span
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${mov.atrasado ? "bg-[#FEF2F2] text-[#DC2626]" : "bg-[#F0FDF4] text-[#16A34A]"}`}
                        >
                          <ArrowDownRight
                            className="w-3 h-3"
                            aria-hidden="true"
                      ...
                      <td className="py-3 px-3 text-right text-[#0F172A] text-sm font-medium whitespace-nowrap">
                        ${mov.monto.toLocaleString("es-CO")}
                      </td>
                      <td className="py-3 px-3 text-right text-[#64748B] text-sm hidden md:table-cell whitespace-nowrap">
