...
export const clientesData: Cliente[] = [
  {
    ...
    infoExtra:
      "Cliente confiable. Familiares: Esposo Juan González. Trabaja en empresa de contabilidad.",
  ...
  },
  {
    ...
    infoExtra:
      "Historial irregular. Hermana: Claudia Martínez (312 000 1111). Precaución con montos altos.",
...
export function calcularEstadoCliente(
  ...
  const deudas = getDeudasByCliente(clienteId).filter(
    (d) => d.estado === "activa",
  );
  if (deudas.some((d) => calcularEstadoDeuda(d) === "atrasado"))
    return "atrasado";
